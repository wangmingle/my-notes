...
start() ->
    [code:add_patha(module_ebin_dir(Module))
     || {Module, _} <- installed()],
    application:start(inets), % 这个是干什么?
...
% 命令对应的函数
% modules_update_specs update
% modules_available available_command
% modules_installed installed_command
...
add_sources(Path) when is_list(Path) ->
    ...
    % add_sources/2/2 跳到这 add_sources/2/2
add_sources(_, "") ->
    {error, no_url};
% add_sources/2/1 (atom,<<"">>) 也会走到 add_sources/2/2
...
% http://erlang.org/doc/man/filename.html#basename-2
% basename -> 最后一个
% filename:rootname("/beam.src/foo.erl"). => "/beam.src/foo"
module_name(Id) ->
